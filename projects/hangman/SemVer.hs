...
parseSemVer = do
    ...
    char '.'
    patch <- integer
    release <- option [] $ char '-' >> sepBy1 parseNos (char '.')
    metadata <- option [] $ char '+' >> sepBy1 parseNos (char '.')
    return $ SemVer major minor patch release metadata

parseNos :: Parser NumberOrString
parseNos = do
    chr <- some alphaNum
    let ints = readMaybe chr :: Maybe Integer
    case ints of
         Just i -> return $ NOSI i
         Nothing -> return $ NOSS chr
